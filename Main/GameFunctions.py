...
class GameFunctions:
    ...
    def drag_unit(self, source_cell, dest_cell):
        ...
        source_coord = config.UNIT_CELL_CENTERS.get(source_cell)
        dest_coord = config.UNIT_CELL_CENTERS.get(dest_cell)
        if source_coord and dest_coord:
            print(f"[드래그] {source_cell} → {dest_cell} 이동")
            # 거리 계산
            distance = dist(source_coord, dest_coord)
            duration = max(0.4, distance / 500)  # 거리 500px 당 최소 1초
    ...
    def hero_roulette(self, elapsed_time=None):
        print("영웅 룰렛 소환 시작")

        base_path = r"C:/Users/user/Desktop/LuckyG/config/unit_image"
        roulette_template = rf"{base_path}/roulette.png"


        #룰렛 닫기 강화 닫기
        close_x, close_y = config.closeRoulette
        pyautogui.click(close_x, close_y)
        #수정예정
        close_region = config.REGIONS['enforceClose']
        close_x = close_region['left'] + close_region['width'] // 2
        close_y = close_region['top'] + close_region['height'] // 2
        pyautogui.click(close_x, close_y)

        # 1. roulette 이미지 클릭
        found = find_image_on_screen(roulette_template, threshold=0.8)
        if found:
            x, y = found
            pyautogui.click(x, y)
            print("룰렛 클릭 완료")
        ...
        for i in range(12):
            ...
            pyautogui.click(config.heroRoulette)
    ...
    def enforce(self, is_first_call=False):
        print("강화 시작")

        base_path = r"C:/Users/user/Desktop/LuckyG/config/unit_image"
        enforce_template = rf"{base_path}/enforce.png"
        ...
        import cv2
        template_image = cv2.imread(enforce_template)
        if template_image is None:
            print(f"[오류] enforce 이미지 로드 실패: {enforce_template}")
            return
        template_height, template_width = template_image.shape[:2]

        # enforce 이미지 클릭 (중앙값 클릭)
        found = find_image_on_screen(enforce_template, threshold=0.8)
        if found:
            x, y = found
            center_x = x + template_width // 2
            center_y = y + template_height // 2
            pyautogui.click(center_x, center_y)
            print(f"enforce 중앙 클릭 완료 ({center_x}, {center_y})")
            time.sleep(1)
        else:
            print("enforce 이미지를 찾지 못했습니다.")
            return

        # 좌표 계산
        lucky_region = config.REGIONS['luckyEnforce']
        lucky_x = lucky_region['left'] + lucky_region['width'] // 2
        lucky_y = lucky_region['top'] + lucky_region['height'] // 2

        legend_region = config.REGIONS['legendEnforce']
        legend_x = legend_region['left'] + legend_region['width'] // 2
        legend_y = legend_region['top'] + legend_region['height'] // 2
        ...
        if is_first_call:
            ...
            if true_count >= 2:
                ...
                for i in range(10):
                    pyautogui.click(legend_x, legend_y)
                    print(f"luckyEnforce 클릭 {i + 1}회")
                    time.sleep(0.3)
            else:
                print("조건 불충족 → luckyEnforce 10회 클릭")
                for i in range(10):
                    pyautogui.click(lucky_x, lucky_y)
                    print(f"luckyEnforce 클릭 {i + 1}회")
                    time.sleep(0.3)
        else:
