...
def find_image_on_screen(template_path, threshold=0.8):
    """
    전체 화면(전체 데스크탑)에서 주어진 템플릿 이미지(template_path)를 찾습니다.
    임계값 이상이면 매칭된 영역의 좌측 상단 좌표를 반환, 아니면 None 반환.
    """
    with mss.mss() as sct:
    ...
    print(f"{template_path} 템플릿 매칭 최대 점수: {max_val}")
...
def click_regions(sequence):
    ...
    for region_name in sequence:
        if region_name in config_center.REGION_CENTERS:
            x, y = config_center.REGION_CENTERS[region_name]
